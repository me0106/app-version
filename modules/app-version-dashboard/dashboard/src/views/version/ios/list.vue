                ...
                <Form :model="queryParams" :label-width="80">
                    ...
                    <Row>
                        <Col span="24" style="text-align: right">
                            <Button type="default" icon="md-add" style="margin-right: 14px" @click="toCreatePage">添加 iOS 版本</Button>
                            <Button type="primary" icon="md-search" @click="searchIOSs">搜索</Button>
                        </Col>
                    </Row>
                </Form>
            </div>
...
export default {
                    ...
                    render: (h, params) => {
                        return h('div', [
                            h('i-switch', {
                                props: {
                                    value: !(
                                        params.row.versionStatus === 0 ||
										params.row.versionStatus === 2
                                    )
                                },
                                on: {
                                    'on-change': value => {
    ...
    methods: {
        ...
        async putChangeStatus (id, status) {
            let response = await http.put('/ios/' + id + '/' + status);
        ...
        async delIOS (id) {
            let response = await http.delete('/ios/' + id);
            if (response.data.code === 200) {
                this.$Notice.success({
                    title: '请求成功',
